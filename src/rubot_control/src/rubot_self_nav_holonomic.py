...
class rUBot:
    ...
    def callbackLaser(self, scan):
        ...
        if closestDistance < self._distanceLaser * 1.1:
            if -45 <= angleClosestDistance <= 45:
                # If the robot is too close to the wall, stop it
                if closestDistance <= self._distanceLaser*0.9:  
                    self._msg.linear.x = 0  # Stop the robot's backward motion
                    rospy.logwarn("Close to the wall. Adjusting robot's movement...")
